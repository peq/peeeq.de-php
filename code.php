...
{
	$pageTitle = htmlspecialchars($_GET['l']).'-Code posten';
...
require_once('SmartTextProcessor.class.php');
...
if (isset($_POST['code']))
{
	...
	if (strlen(trim($code)) < 5) {
		die('Spam verdacht: Code muss mindestends 5 Zeichen lang sein.');
	}

	//check if code contains url:
	if (strpos($code, "http://") !== false) {
		if (session_get_userid_secure() <= 0) {
			die('Um Spam zu vermeiden dürfen Posts, die Internet-Addressen enthalten nur von angemeldeten Benutzern erstellt werden.');
		}
	}
	...
	*/
	...
	$query = 'INSERT INTO 
			  `mapping_nopaste_jass` (`id`, `jass_code`, `datum`, `userid`, `codetype`, `key` )
			  VALUES ("'.$id.'", "'.mysql_real_escape_string($code).'", NOW(), "'.session_get_userid_secure().'", "'.$code_type.'", "'.$key.'")
			  ';
	...
	*/
	...
	{
		$additional_headers[] ='<script>document.location="code.php?id='.$id.$keystring.'";</script>';
	}
	$smarty->assign('id', $id);
...
elseif (isset($_GET['id']))
{
	...
	if ($key != $row['key']) {
		die('Dieser Code ist durch ein Passwort gesch&uuml;tzt. Bitte pr&uuml;fe deinen Link.');
	}

	$code = $row['jass_code'] ;
	$default_lang = $row['codetype'];
	
	$ech = null; //new easycodehighlighter($code, $default_lang, $language_id, $style_id, $format_output);
	if ($language_id == 11 || $language_id == 0 && $default_lang == 'Markdown') {
		$ech = new SmartTextProcessor($code);
	} else {
	...
	if ($key) {	
		$smarty->assign('keyString', "&key=$key");
	}	
	
	
	$pageTitle =  $language_name.'-Code #'.$id.' '.getCodeTitle($code);
...
else
{
	if (isset($_GET['copy'])) {
		...
		$query = 'SELECT * FROM `mapping_nopaste_jass` WHERE `id` LIKE "'.$copyId.'";';
		...
		$smarty->assign('code', strip_markup($row['jass_code']));
	}
...
finish($smarty, "code.tpl", $additional_headers);
